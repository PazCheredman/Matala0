...
public class rowMeasurement {
	...
	/**
	 * checks if the csv table is set by the needed order, otherwise throws error input exception
	 * @param tempfilesArray
	 * @param id
	 * @throws ErrorInInputEception
	 */
	public rowMeasurement(List<String> tempfilesArray,String id) throws ErrorInInputException{
		boolean ok=setAll(tempfilesArray.get(3), id, tempfilesArray.get(6),
						 tempfilesArray.get(7), tempfilesArray.get(8),
						 tempfilesArray.get(1), tempfilesArray.get(0),
						 tempfilesArray.get(4), tempfilesArray.get(5));
		if (!ok){
